...
export function RaInstrumentMatrixTab({ plan }: RaInstrumentMatrixTabProps) {
  ...
  if (ras.length === 0) {
    return (
      <div className="py-12 text-center text-sm text-zinc-400">
        No hay Resultados de Aprendizaje definidos en esta programación.
      </div>
    );
  }
  ...
  if (instruments.length === 0) {
    return (
      <div className="py-12 text-center text-sm text-zinc-400">
        No hay instrumentos de evaluación definidos todavía.
      </div>
    );
  }

  // Para cada RA, calcular la cobertura total y la cobertura por instrumento
  ...
  // Solo mostrar instrumentos que cubren al menos un RA
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5">
            Cobertura de cada Resultado de Aprendizaje por instrumento. El porcentaje indica qué parte de la nota del RA aporta cada instrumento.
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 dark:bg-zinc-900/50">
              <tr className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">
                ...
                {/* One column per active instrument */}
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {raRows.map(({ ra, coverageByInstrument, totalCoverage }) => {
                ...
                const isUnder = totalCoverage < 100 - 0.1;
                ...
                return (
                  <tr key={ra.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30">
                    ...
                    {activeInstruments.map(inst => {
                      ...
                      return (
                        <td key={inst.id} className="px-3 py-3 text-center">
                          {percent > 0 ? (
                            <span className="inline-flex items-center justify-center rounded px-2 py-0.5 text-xs font-semibold tabular-nums bg-zinc-100 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
