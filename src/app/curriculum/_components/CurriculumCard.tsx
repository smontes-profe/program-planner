...
// Component for individual curriculum card with action buttons
export function CurriculumCard({ 
  ...
  const handleArchive = async () => {
    if (!confirm(`¿Archivar el currículo "${template.module_name}"? Esta acción no se puede deshacer.`)) {
      return;
    }
    ...
    try {
      ...
      alert('Error al archivar el currículo');
    }
  };
      ...
      <CardHeader className="space-y-1 pb-2">
        <div className="flex justify-between items-start">
          <Link 
            ...
            className="flex-1"
          >
            <CardTitle className="text-sm font-semibold tracking-tight leading-snug text-zinc-900 dark:text-zinc-50 hover:text-primary transition-colors" title={template.module_name}>
