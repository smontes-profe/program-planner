...
- [P1][x] Programaciones: Al crear una programación, a la hora de selecionar el currículo base, solo aparecerán los propios (ya sean originales o clones).
- [P1][x] Programaciones: Al crear una programación, añadir un filtro para la selección de currículo base.
- [P1][x] Programaciones: Ya se pueden realizar copias de programaciones. Habría que hacer que las copias sean forzadas a privadas y no se puedan hacer públicas. Y que aparezca la etiqueta de "clon" en la lista de programaciones, como pasa con los  currículos.
- [P1][x] Evaluaciones-Exportación: Habilitar un bloque que sea "Compartir evaluación". Este bloque permitirá invitar a otros usuarios a ver la evaluación. Se mostrará un editor nuevo, donde se puedan ver la lista de emails de usuarios con acceso a la evaluación (con posibilidad de eliminarlos), y un campo de texto para añadir nuevos emails separados por comas. Si se añade algún email ya existente, se lanza un mensaje de aviso y no se duplica. Si se añade un email no válido, se lanza un aviso y no se añade. También un texto explicativo: "Añade los emails de los usuarios que quieras invitar, separados por comas. Esos usuarios podrán ver tus evaluaciones pero no modificarlas. Esto es útil, por ejemplo, para dar visibilidad a tus evaluaciones a tutores, jefes de estudio, etc."
- [P0][ ] Programaciones: El "reordenado manual" no está funcionando. Cuando lo suelto, no me respeta donde lo he soltado. Además, cuando modifico alguna propiedad de algún instrumento (como el tipo), se me desordena. Que respete el orden original cuando modifique algo.
...
- [P3][ ] Evaluaciones y Programaciones: Que solo se puedan crear programaciones en base a currículos publicados (además de propios, ya sean normales o clonados). Que solo se puedan montar evaluaciones en base a progarmaciones publicadas (además de propias, ya sean normales o clonadas).
- [P3][ ] Programaciones: que las programaciones clonadas, sí se puedan publicar. Quitamos esta opción, pero como hemos hecho que solo las programaciones publicadas se puedan usar para evaluaciones...
